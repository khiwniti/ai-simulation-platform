    ...
    async def _get_suggestions_impl(
        ...
        try:
            ...
            system_prompt = self._get_system_prompt(
                request.suggestion_type, request.language
            )
            ...
            response = await self.client.messages.create(
                ...
                max_tokens=min(
                    self.config.max_tokens, self._get_max_tokens_for_model()
                ),
...
The cursor is at the position marked with │. Provide helpful insights and suggestions based on the code.{additional_info}"""
    ...
    def _parse_response(
        self, response: Any, request: CodeSuggestionRequest
    ) -> List[CodeSuggestion]:
    ...
    def _parse_json_response(
        self, parsed: Dict[str, Any], request: CodeSuggestionRequest
    ) -> List[CodeSuggestion]:
