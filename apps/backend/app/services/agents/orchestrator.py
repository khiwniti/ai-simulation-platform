...
class AgentOrchestrator:
    ...
    async def broadcast_context_update(
        self, session_id: UUID, context_update: Dict[str, Any]
    ) -> None:
    ...
    async def _select_agents(self, request: CoordinationRequest) -> List[BaseAgent]:
        ...
        for agent_type in team_composition["recommended_types"]:
            type_agents = [
                a for a in agent_registry.get_all_agents() if a.agent_type == agent_type
            ]
            candidates.extend(type_agents)

        # Remove duplicates
        unique_candidates = list(
            {agent.agent_id: agent for agent in candidates}.values()
        )
        ...
        for agent in unique_candidates:
            try:
                ...
                compatibility_score = agent.can_handle_query(
        ...
        selected = await self._assemble_optimal_team(
            scored_agents, request, team_composition
        )
        ...
        logger.info(
            f"Assembled team of {len(selected)} agents for {team_composition['complexity_level']} complexity query"
        )
        return selected

    async def _analyze_optimal_team_composition(
        self, request: CoordinationRequest
    ) -> Dict[str, Any]:
        ...
        physics_score = sum(
            1 for indicator in physics_indicators if indicator in query_lower
        )
        viz_score = sum(1 for indicator in viz_indicators if indicator in query_lower)
        opt_score = sum(
            1 for indicator in optimization_indicators if indicator in query_lower
        )
        debug_score = sum(
            1 for indicator in debug_indicators if indicator in query_lower
        )
        ...
        if complexity_level == "high" and len(recommended_types) >= 2:
            ...
            if (
                "physics" in recommended_types
                and "optimization" not in recommended_types
            ):
                if len(recommended_types) < max_agents:
                    recommended_types.append("optimization")
            if (
                "visualization" in recommended_types
                and "physics" not in recommended_types
            ):
                if len(recommended_types) < max_agents:
    ...
    def _calculate_team_synergy(
        self, agent: BaseAgent, current_team: List[Tuple[BaseAgent, float]]
    ) -> float:
    ...
    def _calculate_specialization_bonus(
        self, agent: BaseAgent, complexity_level: str
    ) -> float:
    ...
    def _calculate_performance_score(self, agent: BaseAgent) -> float:
        ...
        performance_score = (
            confidence_score * 0.4 + time_score * 0.3 + success_rate * 0.3
        )
            ...
            for agent_type in team_composition["recommended_types"]:
                ...
                for agent, score in scored_agents:
                    if (
                        ...
                        and agent.agent_type not in used_types
                    ):
    ...
    async def _safe_agent_query(
        self, agent: BaseAgent, query: str, context: AgentContext
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if not await self._check_agent_health(agent):
                    ...
                    if attempt < max_retries:
                        ...
                        return await self._create_fallback_response(
                            agent, query, "health_check_failed"
                        )
                ...
                if not self._validate_response_quality(response):
                    logger.warning(
                        f"Agent {agent.agent_id} returned low-quality response"
                    )
                    if attempt < max_retries:
                        await asyncio.sleep(retry_delay * (attempt + 1))
                        continue
                ...
                logger.error(
                    f"Agent {agent.agent_id} query timed out (attempt {attempt + 1})"
                )
                if attempt < max_retries:
                    await asyncio.sleep(retry_delay * (attempt + 1))
                    continue
                ...
                logger.error(
                    f"Agent {agent.agent_id} query failed (attempt {attempt + 1}): {e}"
                )
                if attempt < max_retries:
                    await asyncio.sleep(retry_delay * (attempt + 1))
                    continue
    ...
    def _validate_response_quality(self, response: AgentResponse) -> bool:
        ...
        try:
            ...
            error_count = sum(
                1 for indicator in error_indicators if indicator in response_lower
            )
            if error_count >= 2:
                return False
    ...
    async def _create_fallback_response(
        self, agent: BaseAgent, query: str, failure_reason: str
    ) -> AgentResponse:
    ...
    async def _handle_coordination_failure(
        self, request: CoordinationRequest, partial_responses: List[AgentResponse]
    ) -> CoordinationResult:
        ...
        if partial_responses:
            ...
            emergency_response = AgentResponse(
                ...
                response=(
                    "I apologize, but all specialized agents are currently unavailable. "
    ...
    async def _process_with_agents(
        self, agents: List[BaseAgent], query: str, context: AgentContext
    ) -> List[AgentResponse]:
        ...
        for agent in agents:
            task = asyncio.create_task(self._safe_agent_query(agent, query, context))
            tasks.append(task)
        ...
        try:
            ...
            valid_responses = [
                r for r in responses if isinstance(r, AgentResponse) and r is not None
            ]
    ...
    def _detect_conflicts(self, responses: List[AgentResponse]) -> List[Dict[str, Any]]:
        ...
        for i, response1 in enumerate(responses):
            for j, response2 in enumerate(responses[i + 1 :], i + 1):
                ...
                confidence_diff = abs(
                    response1.confidence_score - response2.confidence_score
                )
    ...
    def _find_conflicting_suggestions(
        self, suggestions1: List[str], suggestions2: List[str]
    ) -> List[Dict[str, str]]:
        ...
        conflict_patterns = [
            (
                ["increase", "higher", "more", "boost"],
                ["decrease", "lower", "less", "reduce"],
            ),
        ...
        for s1 in suggestions1:
            for s2 in suggestions2:
                ...
                for positive_terms, negative_terms in conflict_patterns:
                    if any(term in s1_lower for term in positive_terms) and any(
                        term in s2_lower for term in negative_terms
                    ):
                        conflicts.append(
                            {
                                "suggestion1": s1,
                        ...
                        break
                    elif any(term in s1_lower for term in negative_terms) and any(
                        term in s2_lower for term in positive_terms
                    ):
                        conflicts.append(
                            {
                                "suggestion1": s1,
    ...
    def _detect_code_conflicts(
        self, snippets1: List[str], snippets2: List[str]
    ) -> List[Dict[str, str]]:
    ...
    def _assess_conflict_severity(
        self, conflicting_suggestions: List[Dict[str, str]]
    ) -> str:
    ...
    def _suggest_resolution_strategy(
        self, response1: AgentResponse, response2: AgentResponse, conflict_type: str
    ) -> str:
            ...
            try:
                if resolution_strategy.startswith("favor_agent_"):
                    ...
                    hybrid_response = self._create_hybrid_response(
                        result.primary_response, result.supporting_responses
                    )
    ...
    def _create_hybrid_response(
        self, primary: AgentResponse, supporting: List[AgentResponse]
    ) -> AgentResponse:
        ...
        hybrid_text += (
            f"Primary analysis ({primary.agent_type}): {primary.response}\n\n"
        )

        for response in supporting:
            hybrid_text += (
                f"Supporting analysis ({response.agent_type}): {response.response}\n\n"
            )
        ...
        weighted_confidence = (
            total_weight / len(all_responses) if all_responses else 0.0
        )
        ...
        return AgentResponse(
            ...
            capabilities_used=list(
                set().union(*[r.capabilities_used for r in all_responses])
            ),
    ...
    def _update_coordination_metrics(
        self, result: Optional[CoordinationResult], success: bool
    ) -> None:
        ...
        if success:
            ...
            if result:
                ...
                new_avg = (
                    (current_avg * (total - 1)) + result.coordination_time
                ) / total
                ...
                if result.conflicts:
                    conflicts_resolved = len(
                        [c for c in result.conflicts if "resolved" in c]
                    )
    ...
    def get_coordination_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        recent = (
            self.coordination_history[-limit:]
            if limit > 0
            else self.coordination_history
        )
